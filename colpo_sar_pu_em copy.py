...
def train_val(outer_epochs, train_loader, val_loader, train_writer, val_writer):
    for epoch in range(outer_epochs):
        backbone_loop(train_loader, train_writer)
        backbone_loop(val_loader, val_writer)
        
def backbone_loop(backbone, optim, c_model, p_model, dataloader):
    downstream_loop()
    toggle_downstream(c_model, p_model, False)
    for n_iter, (x, labels) in enumerate(dataloader):
        features = backbone(x)
        ...
        optim.zero_grad()
        loss = loglikelihood(exp_prior_y1, exp_propensity, labels)
        loss = torch.mean(loss)
        loss.backward()
        optim.step()


def downstream_loop(backbone, c_model, p_model, c_optim, p_optim, c_criterion, 
                    p_criterion, dataloader, epochs):
    for epoch in range(epochs):
        for n_iter, (x, s) in enumerate(dataloader):
            ...
            p_loss = p_criterion(exp_propensity, s, sample_weight=exp_post_y1)
            p_loss.backward()
            p_optim.step()
...
def loglikelihood(pred, prop, labels):

    #pred = torch.nan_to_num(pred)
    #prop = torch.nan_to_num(prop)
